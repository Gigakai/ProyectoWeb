...
function CategoryCard(props){
    ...
    return (
        <div
            className="text-black w-full bg-[#F5F5F5] rounded-xl shadow-[1px_10px_10px_rgba(0,0,0,0.3)] min-h-[10rem] pb-4 hover:shadow-lg transition"
        >
            <div className="space-y-4 w-full px-4 py-1 gap-1">
                <div className="flex flex-row w-full justify-between">
                    <h3 className="font-semibold text-start text-3xl w-full">{props.name}</h3>
                    <div className="w-full flex justify-end items-center gap-1 flex-row justify-center">
                        No. Asociados:
                        <span className="text-lg">{props.numProducts}</span>
                    </div>
                </div>
                <span
